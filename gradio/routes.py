...
class App(FastAPI):
    ...
    def create_app(
        ...
        def main(
            ...
            # Try multiple strategies to locate the templates directory.
            # When installed from source (pip install -e .), importlib.resources
            # may return a Traversable that Jinja2's FileSystemLoader cannot use.
            template_dirs_to_try = [STATIC_TEMPLATE_LIB]

            # Fallback 1: resolve relative to this source file
            from pathlib import Path as _Path

            source_dir = _Path(__file__).resolve().parent / "templates"
            source_dir_str = source_dir.as_posix()
            if source_dir_str != STATIC_TEMPLATE_LIB:
                template_dirs_to_try.append(source_dir_str)

            # Fallback 2: resolve via the gradio package's __file__
            try:
                import gradio as _gradio

                pkg_dir = _Path(_gradio.__file__).resolve().parent / "templates"
                pkg_dir_str = pkg_dir.as_posix()
                if pkg_dir_str not in template_dirs_to_try:
                    template_dirs_to_try.append(pkg_dir_str)
            except Exception:
                pass

            for template_dir in template_dirs_to_try:
                try:
                    if template_dir != STATIC_TEMPLATE_LIB:
                        tpl = Jinja2Templates(directory=template_dir)
                        tpl.env.filters["toorjson"] = toorjson
                    else:
                        tpl = templates
            ...
            # All strategies failed — give a clear error with diagnostic info
            tried = ", ".join(template_dirs_to_try)
            raise ValueError(
                f"Template '{template}' not found. Searched directories: [{tried}]. "
                f"If you installed Gradio from source, please build the frontend "
                f"first by running: bash scripts/build_frontend.sh"
            )
