...
def _resolve_gradio_path(*subpath: str) -> str:
    """Resolve a path within the gradio package, with fallback for editable installs.

    When Gradio is installed from source (pip install -e .), importlib.resources
    may return a Traversable that doesn't work well with Jinja2's filesystem loader.
    This function uses __file__-based resolution as the primary strategy (most
    reliable for editable installs) and falls back to importlib.resources.
    """
    # Primary: resolve relative to this file's location (works for both
    # pip install -e . and regular pip install from source)
    base = Path(__file__).resolve().parent
    for part in subpath:
        base = base / part
    result = base.as_posix()
    if os.path.isdir(result):
        return result

    # Fallback: try importlib.resources (works for PyPI wheel installs)
    try:
        resolved = importlib.resources.files("gradio")
        for part in subpath:
    ...
    # Return the __file__-based path even if it doesn't exist yet
    # (the directory might be created later by build_frontend.sh)
    return result


STATIC_TEMPLATE_LIB = cast(DeveloperPath, _resolve_gradio_path("templates"))
STATIC_PATH_LIB = cast(
    DeveloperPath, _resolve_gradio_path("templates", "frontend", "static")
)
BUILD_PATH_LIB = cast(
    DeveloperPath, _resolve_gradio_path("templates", "frontend", "assets")
)
