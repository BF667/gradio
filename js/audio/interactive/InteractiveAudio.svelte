<script lang="ts">
        ...
        import { init_media_recorder, assembleWavBlob } from "../streaming/media_recorder";
        ...
        async function handle_chunk(event: IBlobEvent): Promise<void> {
                ...
                if (pending) {
                        ...
                        // Use optimized blob assembly (Web Worker for large payloads)
                        const wavBlob = await assembleWavBlob(header, pending_stream, payload);
                        dispatch_blob([wavBlob], "stream");
                        pending_stream = [];
                }
        }
        ...
        $effect(() => {
                if (submit_pending_stream_on_pending_end && pending === false) {
                        ...
                        if (header && pending_stream.length > 0) {
                                const pendingCopy = [...pending_stream];
                                pending_stream = [];
                                // Use optimized blob assembly
                                assembleWavBlob(header, pendingCopy, new Uint8Array(0)).then((wavBlob) => {
                                        dispatch_blob([wavBlob], "stream");
                                });
                        }
                }
