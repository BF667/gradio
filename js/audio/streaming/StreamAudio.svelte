<script lang="ts">
	...
	import { loadWaveSurfer, loadRecordPlugin } from "../shared/lazy";
	...
	let micWaveform: InstanceType<WaveSurferType> | undefined;
	let waveformRecord: any;
	...
	const create_mic_waveform = async (): Promise<void> => {
		if (micWaveform !== undefined) micWaveform.destroy();
		if (!microphoneContainer) return;

		// Lazy-load WaveSurfer and RecordPlugin
		const { WaveSurfer } = await loadWaveSurfer();
		const { RecordPlugin } = await loadRecordPlugin();
