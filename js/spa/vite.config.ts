...
logger.warn = (log, options) => {
	...
	if (log.includes("[vue]")) return false;
	...
	return {
		...
		build: {
			...
			rollupOptions: {
				...
				output: {
					// Code-splitting: separate chunks for heavy audio deps and Vue/PrimeVue
					manualChunks(id) {
						...
						if (
							id.includes("extendable-media-recorder")
						) {
							return "vendor-media-recorder";
						}
					}
				}
			}
					...
					prefixer({
						...
						transform(prefix, selector, prefixedSelector, fileName) {
							if (selector.indexOf("gradio-container") > -1) {
								...
								selector.indexOf("p-") > -1 || // PrimeVue class prefix
								selector.indexOf("primevue") > -1 ||
		...
		plugins: [
			handle_msw_imports(),
			// Vue plugin MUST come before Svelte plugin so .vue files are handled
			// before Svelte tries to process them
			vue({
				// Enable custom element mode for PrimeVue bridge components
				customElement: /primevue-bridge/,
				// Don't process .svelte files
				include: [/\.vue$/],
				template: {
					compilerOptions: {
						// Treat PrimeVue components as custom elements
						isCustomElement: (tag: string) =>
							tag.startsWith("primevue-")
					}
				}
			}),
			svelte_plugin({
